...
def play_match(env, agent_a, agent_b, num_hands: int):
    ...
    根據 env.state.current_player 路由，局末根據 get_reward 分別結算。
    """
    rewards_a, rewards_b = [], []
...
def play_match_meta(env, meta, opponent, num_hands: int):
    """meta = player 0, opponent = player 1."""
    rewards_meta = []
    all_weights = []
    for _ in range(num_hands):
        obs = env.reset()
...
def evaluate_subagents(checkpoint_dir, num_hands=1000):
    ...
    for a_name, b_name in pairs:
        print(f"\nRunning: {a_name} vs {b_name} ({num_hands} hands)...", flush=True)
    ...
    print("-" * 60)
    for i, name in enumerate(sorted(names, key=lambda n: np.mean(results[n]), reverse=True)):
...
def evaluate_meta(checkpoint_dir, num_hands=1000):
    ...
    for opp_name, opp_agent in sub_agents.items():
        print(f"\nRunning: Meta vs {opp_name} ({num_hands} hands)...", flush=True)
